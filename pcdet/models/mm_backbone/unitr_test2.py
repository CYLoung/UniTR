...
#         return batch_dict
class UniTR(nn.Module):
    def __init__(self, model_cfg, use_map=False, num_frames=3, **kwargs):
        super().__init__()
        ...
        self.accelerate = self.model_cfg.get('ACCELERATE', False)
        self.use_map = use_map
        self.num_frames = num_frames  # Temporal Frame 수 설정
        ...
        # Sparse Regional Attention Blocks 설정
        dim_feedforward = self.model_cfg.dim_feedforward
        dropout = self.model_cfg.dropout
        activation = self.model_cfg.activation
        layer_cfg = self.model_cfg.layer_cfg
    ...
    def forward(self, batch_dict, previous_frames=None):
        '''
        Args:
            batch_dict (dict): 현재 프레임 데이터
            previous_frames (list[dict]): 이전 프레임 데이터의 리스트

        Returns:
            batch_dict (dict): 업데이트된 배치 딕셔너리
        '''
        print("\n [1] forward time \n")
        # 이전 프레임 정보가 있다면 이를 통합
        if previous_frames is not None:
            # 이전 프레임들과 현재 프레임의 데이터를 함께 처리

            combined_frames = [batch_dict] + previous_frames
            combined_output = self.temporal_forward(combined_frames)
            batch_dict.update(combined_output)
            return batch_dict
        else:
            return self.single_frame_forward(batch_dict)

    def single_frame_forward(self, batch_dict):
        '''
        현재 프레임에 대해 단일 프레임 처리를 수행합니다.
        '''
        # 기존의 forward 처리 로직과 유사
        multi_feat, voxel_info, patch_info, multi_set_voxel_inds_list, multi_set_voxel_masks_list, multi_pos_embed_list = self._input_preprocess(
            batch_dict)
        
        # Fuse Backbone 등을 통해 데이터를 처리
        output = multi_feat
        block_id = 0
        voxel_num = batch_dict['voxel_num']
        batch_dict['image_features'] = []
    ...
    def temporal_forward(self, frames):
        '''
        이전 프레임과 현재 프레임을 통합하여 처리하는 Temporal Forward 로직.
        frames: 이전 프레임들과 현재 프레임의 리스트.
        '''
        # Temporal Deformable Attention 등을 사용하여 프레임들을 통합 처리
        combined_output = {}
        for frame in frames:
            output = self.single_frame_forward(frame)
            # 각 프레임의 output을 원하는 방식으로 통합
            # 예: 평균, 가중합, 혹은 Deformable Attention을 통한 통합
            for key in output:
                if key not in combined_output:
                    combined_output[key] = output[key].unsqueeze(0)
                else:
                    combined_output[key] = torch.cat(
                        [combined_output[key], output[key].unsqueeze(0)], dim=0)
        
        # 필요한 경우 Temporal Deformable Attention 적용
        # combined_output = self.temporal_attention(combined_output)
        
        # 최종 통합 결과 반환
        return {key: torch.mean(value, dim=0) for key, value in combined_output.items()}
    ...
    def forward(
        ...
        for i in range(num_shifts):
            ...
            if using_checkpoint and self.training:
                output = checkpoint(
                    layer, output, set_voxel_inds, set_voxel_masks, pos_embed, voxel_num, previous_frames)
            else:
                output = layer(output, set_voxel_inds,
                               set_voxel_masks, pos_embed, voxel_num=voxel_num, previous_frames=previous_frames)
...
class UniTR_EncoderLayer(nn.Module):
    def __init__(self, d_model=128, nhead=8, dim_feedforward=256, dropout=0.1, activation="relu", batch_first=True, layer_cfg=dict()):
        super(UniTR_EncoderLayer, self).__init__()
    ...
    def forward(self, src, set_voxel_inds, set_voxel_masks, pos_embed, voxel_num=0, previous_frames=None):
        """
        Args:
            src (torch.Tensor): 현재 프레임의 입력 텐서
            set_voxel_inds (torch.Tensor): voxel 인덱스
            set_voxel_masks (torch.Tensor): voxel 마스크
            pos_embed (torch.Tensor): 포지션 임베딩
            voxel_num (int): voxel 수
            previous_frames (list of torch.Tensor): 이전 프레임들의 리스트

        Returns:
            torch.Tensor: attention 결과
        """
        if previous_frames is not None:
            combined_frames = torch.cat([src] + previous_frames, dim=0)
            output = self.self_attn(combined_frames, set_voxel_inds, set_voxel_masks, pos_embed)
        else:
            output = self.self_attn(src, set_voxel_inds, set_voxel_masks, pos_embed)
...
#     def forward(self, current_frame, previous_frames=None):
#         """
#         Args:
#             current_frame (torch.Tensor): 현재 프레임의 입력 텐서
#             previous_frames (list of torch.Tensor): 이전 프레임의 텐서 리스트

#         Returns:
#             torch.Tensor: attention 결과
#         """
#         # 이전 프레임이 주어졌을 때, 현재 프레임과 결합하여 attention 수행
#         if previous_frames is not None:
#             # 이전 프레임들과 현재 프레임을 하나의 텐서로 결합
#             combined_frames = torch.cat([current_frame] + previous_frames, dim=0)
#             output = self.self_attn(combined_frames)
#         else:
#             # 이전 프레임이 없는 경우, 현재 프레임만으로 attention 수행
#             output = self.self_attn(current_frame)

#         # Residual connection and layer normalization
#         src = current_frame + self.dropout1(output)
#         src = self.norm1(src)
#         src2 = self.linear2(self.dropout(self.linear1(src)))
#         src = src + self.dropout2(src2)
#         src = self.norm2(src)
...
########### 2 ########################################
# class UniTR_EncoderLayer(nn.Module):
#     def __init__(self, d_model=128, nhead=8, dim_feedforward=256, dropout=0.1, num_frames=3):
#         super(UniTR_EncoderLayer, self).__init__()
        
#         # DeformableAttention 모듈 초기화
#         self.temporal_attn = DeformableAttention(
#             dim=d_model,
#             heads=nhead,
#             dim_head=d_model // nhead,
#             dropout=dropout
...
#     def forward(self, current_frame, previous_frames):
#         """
#         current_frame: 현재 프레임의 입력
#         previous_frames: 이전 프레임들의 리스트
#         """
        
#         # 이전 프레임들과 현재 프레임을 함께 Deformable Attention을 통해 처리
#         all_frames = torch.cat([current_frame] + previous_frames, dim=1)  # 모든 프레임을 하나로 연결
#         output = self.temporal_attn(all_frames)
        
#         # 이어서 기존의 UniTR_EncoderLayer 로직 수행
#         output = self.norm1(output + current_frame)
#         output = self.norm2(self.linear2(self.dropout(self.linear1(output))) + output)
