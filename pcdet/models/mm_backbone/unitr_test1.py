...
class MSDeformAttn(nn.Module):
    ...
    def _reset_parameters(self):
        ...
        constant_(self.value_proj.weight.data, 0.)
        xavier_uniform_(self.output_proj.weight.data)

    def forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask=None):
        """
        Args:
            query: Tensor of shape (N, L_query, C)
            reference_points: Tensor of shape (N, L_query, num_levels, 2), each reference point is (x, y)
            input_flatten: Tensor of shape (N, L_input, C)
            input_spatial_shapes: Tensor of shape (num_levels, 2), [(H1, W1), (H2, W2), ...]
            input_level_start_index: Tensor of shape (num_levels), [0, H1*W1, H1*W1+H2*W2, ...]
            input_padding_mask: Tensor of shape (N, L_input), optional

        Returns:
            output: Tensor of shape (N, L_query, C)
        """
        ...
        if reference_points.shape[-1] == 2:
            ...
            raise ValueError("Last dimension of reference_points must be 2")

        output = MSDeformAttnFunction.apply(value, input_spatial_shapes, sampling_locations, attention_weights)
...
class MSDeformAttnFunction(torch.autograd.Function):
    ...
    def forward(ctx, value, spatial_shapes, sampling_locations, attention_weights):
        N, L_in, heads, C_per_head = value.shape
        L_query = sampling_locations.shape[1]
        num_levels = spatial_shapes.shape[0]

        value = value.view(N, L_in, heads, C_per_head)
        sampling_grids = sampling_locations.view(N, L_query, heads, num_levels, -1, 2)
        sampling_value = MSDeformAttnFunction.bilinear_sampling(value, sampling_grids, spatial_shapes)
        
        output = (sampling_value * attention_weights).sum(dim=-1).sum(dim=-2).view(N, L_query, heads * C_per_head)
        return output

    @staticmethod
    def bilinear_sampling(value, sampling_grids, spatial_shapes):
        N, L_in, heads, C_per_head = value.shape
        num_levels, num_points, _ = sampling_grids.shape[3:6]
        output = torch.zeros_like(value[:, :sampling_grids.shape[1]])

        for lvl in range(num_levels):
            H, W = spatial_shapes[lvl]
            grid_lvl = sampling_grids[:, :, :, lvl, :, :]
            x, y = grid_lvl[..., 0] * W, grid_lvl[..., 1] * H
            x0, x1 = x.floor().long(), x.ceil().long()
            y0, y1 = y.floor().long(), y.ceil().long()
            ...
            output += wa.unsqueeze(-1) * MSDeformAttnFunction.get_value(value, x0, y0, lvl, spatial_shapes)
            output += wb.unsqueeze(-1) * MSDeformAttnFunction.get_value(value, x0, y1, lvl, spatial_shapes)
            output += wc.unsqueeze(-1) * MSDeformAttnFunction.get_value(value, x1, y0, lvl, spatial_shapes)
            output += wd.unsqueeze(-1) * MSDeformAttnFunction.get_value(value, x1, y1, lvl, spatial_shapes)

        return output

    @staticmethod
    def get_value(value, x, y, level, spatial_shapes):
        N, L_in, heads, C_per_head = value.shape
        H, W = spatial_shapes[level]
        index = y * W + x
        index = index.view(N, -1, 1, 1).expand(-1, -1, heads, C_per_head).long()
        return value[:, index]
...
class SetAttention(nn.Module):
    ...
    def __init__(self, d_model, nhead, dropout, dim_feedforward=2048, activation="relu", batch_first=True, mlp_dropout=0, layer_cfg=dict()):
        ...
        # self.deform_attn = MSDeformAttn(d_model, nhead, num_levels=4, num_points=4)  # Adjust num_levels and num_points as needed
