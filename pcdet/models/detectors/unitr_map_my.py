...
class TSALayer(nn.Module):
    def __init__(self, embed_dim, num_heads):
        ...
        self.deformable_attention = nn.MultiheadAttention(embed_dim, num_heads)
        self.norm = nn.LayerNorm(embed_dim)

    def forward(self, current_seq, previous_seq=None):
        if previous_seq is not None:
            attn_output, _ = self.deformable_attention(current_seq, previous_seq, previous_seq)
            return self.norm(attn_output + current_seq)
        else:
            # 이전 시퀀스가 없으면 기본적으로 현재 시퀀스만 반환
            return current_seq
...
class UniTRMAP(Detector3DTemplate):
    def __init__(self, model_cfg, num_class, dataset):
        ...
        #@LCY TSA Layer 추가
        self.tsa_layer = TSALayer(embed_dim=128, num_heads=8)
    ...
    #     previous_seq = None  # 이전 시퀀스를 저장할 변수

    #     for cur_module in self.module_list:
    #         if cur_module == self.mm_backbone:  # mm_backbone 전에 TSA Layer 적용
    #             batch_dict = self.tsa_layer(batch_dict, previous_seq)
    #         batch_dict = cur_module(batch_dict)
    #         previous_seq = batch_dict  # 현재 시퀀스를 이전 시퀀스로 갱신

    #     # BEV Pooling 후 TSA Layer 적용
    #     if 'bev_features' in batch_dict:
    #         batch_dict['bev_features'] = self.tsa_layer(batch_dict['bev_features'], previous_seq)
    ...
    #             current_seq = batch_dict['feature_map']  # batch_dict에서 적절한 텐서를 선택합니다.
    #             batch_dict['feature_map'] = self.tsa_layer(current_seq, previous_seq)
    #         print("\n KEY: " , batch_dict.keys()) 
    #         batch_dict = cur_module(batch_dict)
    #         previous_seq = batch_dict['feature_map']  # 현재 시퀀스를 이전 시퀀스로 갱신
    ...
    def forward(self, batch_dict):
        ...
        # TSA Layer를 적용할 데이터 선택
        current_seq = batch_dict.get('gt_masks_bev', None)  # 'gt_masks_bev'를 TSA Layer에 적용

        if current_seq is not None:
            batch_dict['gt_masks_bev'] = self.tsa_layer(current_seq, previous_seq)

        for cur_module in self.module_list:
            batch_dict = cur_module(batch_dict)
            previous_seq = batch_dict.get('gt_masks_bev', None)  # 'gt_masks_bev'으로 업데이트
